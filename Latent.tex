...
Estimate prior $P(y)$; Est. distr. for each class:
$P(x|y) = P(x|\mu,\Sigma,w) = \sum_{j=1}^{k_y} w_j^{(y)} \mathcal{N}(x; \mu_j^{(y)}, \Sigma_j^{(y)})$\\

\subsection*{Hard-EM algorithm}
Initialize parameters $\theta^{(0)} = [w_0, \mu_0, \Sigma_0]$\\
