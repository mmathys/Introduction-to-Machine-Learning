...
i.e. $k(\mathbf{x}, \mathbf{x'}) = \langle \varphi(\mathbf{x}), \varphi(\mathbf{x'}) \rangle_\mathcal{V} = \varphi(x)^T\varphi(x')$\\
...
$k_1(x,y) + k_2(x,y)$; $k_1(x,y) \cdot k_2(x,y)$; $c \cdot k_1(x,y)$, $c>0$; $f(k_1(x, y))$ where f is poly. with positive coefficients or an exponential, $f(x)k(x, y)f(y)$.
